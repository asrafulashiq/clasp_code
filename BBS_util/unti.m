...
pz = 7; % non-overlapping patch size used for pixel descirption
dataDir ='..\data';
...
Iref = im2single(imresize(imread('mydata\pair001_frm1.jpg'),0.5));
Ir = im2single(imresize(imread('mydata\2330_6A.jpg'),[size(Iref,1) size(Iref,2)]));

rect = double([10 91 152 108]);
% figure;imshow(Iref);
% rect = getrect;
% close all;
%rectGT = [8 154 310 319];
