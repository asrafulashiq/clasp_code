function [R_people, R9_people, events] = people_detector_tracking_camera_2(im_r, im_flow, R_people, R9_people)
...
if ~isempty(R_people.people_array) && ~isempty(list_bbox)
    ...
    for k = del_exit
        ...
        events{end+1}.text = sprintf('P%d exits from scene', R_people.people_array{k}.label);
    end
            ...
            if min_val_ind(2) ~= -1
               ...
               for j=1:numel(R9_people.people_array)
                   if R9_people.people_array{j}.label == Person.label
                       ...
                       events{end+1}.text = sprintf('P%d enters into scene', R_people.people_array{j}.label);
